...
public class FriendsListInterface : BaseFriendInterface {
	...
	protected override void DrawFriendsList(bool loadingSuccess)
	{
		...
		{
			case 0:
				actorList = SUGARManager.Friend.Friends;
				_listTypeText.text = Localization.Get("FRIENDS_LIST");
				break;
			case 1:
				actorList = SUGARManager.Friend.PendingReceived;
				_listTypeText.text = Localization.Get("FRIENDS_PENDING_REQUESTS");
				break;
			case 2:
				actorList = SUGARManager.Friend.PendingSent;
				_listTypeText.text = Localization.Get("FRIENDS_PENDING_SENT");
				break;
			case 3:
				actorList = SUGARManager.Friend.SearchResults;
				_listTypeText.text = Localization.Get("SEARCH");
				_searchArea.SetActive(true);
				_actorItems[0].gameObject.SetActive(false);
				break;
		}
		var length = _listType == 3 ? _actorItems.Length - 1 : _actorItems.Length;
		...
		for (int i = _listType == 3 ? 1 : 0; i < _actorItems.Length; i++)
		{
			if (i - (_listType == 3 ? 1 : 0) >= actorList.Count)
			{
				_actorItems[i].gameObject.SetActive(false);
			}
			else
			{
	...
	{
		GetComponentsInChildren<Button>(true).Where(t => !t.GetComponentInParent<ActorItemInterface>()).Select(t => t.gameObject).BestFit();
	}
